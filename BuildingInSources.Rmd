...
derivNLLMM2MOFFAT <- function(params, x, y){
  ...
  u <- lambda1*c1*v1^(-beta1) + (1 - lambda1)*c2*v2^(-beta2)
...
MMresult <- optim(par = initialParams, fn = NLLMM2MOFFAT, x = interestArea2$x, y = interestArea2$y, method = "Nelder-Mead")
...
```{r}
initialParams <- c(lambda1 = 0.7, x1 = 4115.261, y1 = 4051.904, alpha1 = 0.683, beta1 = 1.9, lambda2 = 0.2, x2 = 4124, y2 = 4063, alpha2 = 0.5, beta2 = 1.9, x3 = 4128, y3 = 4069, alpha3 = 0.5, beta3 = 1.9)

lowerbounds = c(1e-6, 3000, 3000, 1e-6, 1+1e-6, 1e-6, 3000, 3000, 1e-6, 1+1e-6, 3000, 3000, 1e-6, 1+1e-6)
upperbounds = c(1, 5000, 5000, Inf, Inf, 1, 5000, 5000, Inf, Inf, 5000, 5000, Inf, Inf)

MMresult <- optim(par = initialParams, fn = MM3MOFFAT, x = interestArea$x, y = interestArea$y, method = "Nelder-Mead")
...
oparams <- c(out[[1]], out[[2]], out[[3]], out[[4]], out[[5]], out[[6]], out[[7]], out[[8]], out[[9]], out[[10]], 1 - out[[1]] - out[[6]], out[[11]], out[[12]], out[[13]], out[[14]])
zMM <- MM3MOFFATimage(grid, params = oparams)
zmat <- matrix(zMM, nrow = length(xRange), ncol = length(yRange))
par(pty = "s")
image(xRange, yRange, zmat, xlab = "x", ylab = "y", main = "2D Moffat")

contours <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM), bins = 20) +
  coord_fixed()
...
#params
oparams1 <- c(out[[1]], out[[2]], out[[3]], out[[4]], out[[5]])
oparams2 <- c(out[[6]], out[[7]], out[[8]], out[[9]], out[[10]])
oparams3 <- c(1 - out[[1]] - out[[6]], out[[11]], out[[12]], out[[13]], out[[14]])
...
contours3 <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM3), bins = 20) +
  coord_fixed()
...
```{r}
initialParams <- c(lambda1 = 0.6, x1 = 4115.261, y1 = 4051.904, alpha1 = 0.683, beta1 = 1.9,
                   lambda2 = 0.2, x2 = 4124, y2 = 4063, alpha2 = 0.5, beta2 = 1.9,
                   lambda3 = 0.1, x3 = 4128, y3 = 4069, alpha3 = 0.5, beta3 = 1.9,
                   lambda4 = 0.05, x4 = 4128, y4 = 4064, alpha4 = 0.5, beta4 = 1.9,
                   lambda5 = 0.05, x5 = 4129, y5 = 4073, alpha5 = 0.5, beta5 = 1.9
                   )
...
MMresult5 <- optim(par = initialParams, fn = MM5MOFFAT, x = interestArea$x, y = interestArea$y, method = "L-BFGS-B", lower = lowerbounds, upper = upperbounds)
