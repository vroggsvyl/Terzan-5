...
contours <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM2), bins = 20) +
  coord_fixed()
...
contours <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM3), bins = 40) +
  coord_fixed()
...
#params
oparams1 <- c(out3[[1]], out3[[2]], out3[[3]], out3[[4]], out3[[5]])
oparams2 <- c(out3[[6]], out3[[7]], out3[[8]], out3[[9]], out3[[10]])
oparams3 <- c(1 - out3[[1]] - out3[[6]], out3[[11]], out3[[12]], out3[[13]], out3[[14]])
...
contours1 <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM1), bins = 40) +
  coord_fixed()
...
contours3 <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM3), bins = 5) +
  coord_fixed()
...
MMcontourData <- ggplot_build(contours)$data[[1]] #extracting the contours (from the optimised function)
MMcombined <- interestAreaModel3 + geom_path(data = MMcontourData, aes(x = x, y = y, group = group), color = "red", alpha = 0.5)
...
```{r}
initialParams <- c(lambda1 = 0.4, x1 = 4130, y1 = 4095, alpha1 = 0.5, beta1 = 1.5,
                   lambda2 = 0.4, x2 = 4146, y2 = 4092, alpha2 = 0.45, beta2 = 1.3,
...
#build contours
contours1 <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM1), bins = 10) +
  coord_fixed()
...
```{r}
initialParams5 <- c(lambda1 = 0.4, x1 = 4130, y1 = 4095, alpha1 = 0.5, beta1 = 1.5,
                   lambda2 = 0.4, x2 = 4146, y2 = 4092, alpha2 = 0.45, beta2 = 1.3,
                   lambda3 = 0.1, x3 = 4139, y3 = 4099, alpha3 = 0.6, beta3 = 1.4,
...
MMresult5 <- optim(par = initialParams5, fn = MM5MOFFAT, x = interestArea3$x, y = interestArea3$y, method = "L-BFGS-B", lower = lowerbounds, upper = upperbounds)
...
opt5params <- c(out5[[1]], out5[[2]], out5[[3]], out5[[4]], out5[[5]],
                out5[[6]], out5[[7]], out5[[8]], out5[[9]], out5[[10]],
                out5[[11]], out5[[12]], out5[[13]], out5[[14]], out5[[15]],
                out5[[16]], out5[[17]], out5[[18]], out5[[19]], out5[[20]], 
                1 - out5[[1]] - out5[[6]] - out5[[11]] - out5[[16]], out5[[21]], out5[[22]], out5[[23]], out5[[24]])
...
contours <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM5), bins = 10) +
  coord_fixed()
...
MM8MOFFAT(initialParams5, xRange, yRange)
...
contours <- ggplot(grid, aes(x = x, y = y)) + 
  stat_contour(aes(z = zMM5), bins = 20) +
  coord_fixed()
