...
``{r} #results in length errors
MoffatCDF <- function(xRange, PDF, parameters){
  MOFFATCDF <- numeric(length = length(xRange))
  
  for(i in 1:100){
    MOFFATCDF[i] <- integrate(PDF, lower = xRange[1], upper = xRange[i], parameters)$value
  }
  return(MOFFATCDF)
}

xRange <- seq(4105, 4125, length = 100)
MoffatCDF(xRange = xRange, PDF = MOFFATPDF, parameters = optimPAR2)
```
```{r}
xRange <- seq(4105, 4125, length = 100)
...
Define CDF as a function in R
pass ^ for the ks test
