...
interestArea2 <- T5 %>%
  ...
  interestArea3 <- T5 %>%
  filter(x>4125 & x<4150 & y>4075 & y<4100)
...
xy <- expand.grid(datafull[,1], datafull[,2])
...
```
```{r}
NLLMM2MOFFAT(initialParams, x = datafull[,1], y = datafull[,2])
```
```{r}
MM2MOFFAT(initialParams, x = datafull[,1], y = datafull[,2])
```
...
MMresult <- optim(par = initialParams, fn = MM3MOFFAT, x = interestArea$x, y = interestArea$y, method = "Nelder-Mead")#, #lower = lowerbounds, upper = upperbounds)
...
oparams <- c(out[[1]], out[[2]], out[[3]], out[[4]], out[[5]], out[[6]], out[[7]], out[[8]], out[[9]], out[[10]])
zMM <- MM2MOFFATimage(grid, params = oparams)
zmat <- matrix(zMM, nrow = length(xRange), ncol = length(yRange))
par(pty = "s")
image(xRange, yRange, zmat, xlab = "x", ylab = "y", main = "2D Moffat")
