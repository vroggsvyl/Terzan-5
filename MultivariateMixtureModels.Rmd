...
```{r}
Mixed2MOFFAT <- function(parameters, x, y){
  x01 <- parameters[[1]]
  y01 <- parameters[[2]]
  alpha1 <- parameters[[3]]
  beta1 <- parameters[[4]]
...
```{r}
norm1 <- c(mean = 0, sd = 1)
norm2 <- c(mean = 5, sd = 1.3)
norm3 <- c(mean = 7, sd = 0.7)

ggplot() +
  stat_function(fun = gaussian_density, n = 1000, args = list(parameters = norm1), aes(colour = "red")) +
  stat_function(fun = gaussian_density, n = 1000, args = list(parameters = norm2), aes(colour = "blue")) +
  stat_function(fun = gaussian_density, n = 1000, args = list(parameters = norm3), aes(colour = "green")) +
  xlim(-3, 3)
```
...
Chain Rule: $\frac{\delta}{\delta x} = \frac{\delta}{\delta y} \frac{\delta y}{\delta x} $
Quotient Rule: $\frac{\delta}{\delta x} = \frac{ v \frac{\delta u}{\delta x} - { u \frac{\delta v}{\delta x}}}{v^2} $
Product Rule:  $\frac{\delta (uv)}{\delta x} = u \frac{\delta v}{\delta x} + { v \frac{\delta u}{\delta x}}$
...
$$\frac{\delta u}{\delta \alpha_1} = \frac{\lambda_1(\beta_1 -1)}{\pi\alpha_{1}^{2}} \frac{2 \beta_1\left[(x-x_1)^{2}+(y-y_1)^{2}\right]}{v_1^{(\beta_1 +1)}\alpha_1^3} - \frac{2 \lambda_1(\beta_1 - 1)}{\pi\alpha_{1}^{3}} v_1^{-\beta_1}$$
This can be simplified by factoring out the common term $\frac{2\lambda_1(\beta_1 - 1)}{\pi \alpha_1^3 v_1^\beta}$
$$\frac{\delta u}{\delta \alpha_1} = \frac{2\lambda_1(\beta_1 - 1)}{\pi \alpha_1^3 v_1^\beta}(\frac{\beta_1\left[(x-x_1)^{2}+(y-y_1)^{2}\right]}{v_1 \alpha_1^2} - 1)$$
Resulting in
$$\frac{\delta}{\delta \alpha_1} = -\sum \frac {\frac{2\lambda_1(\beta_1 - 1)}{\pi \alpha_1^3 v_1^\beta}(\frac{\beta_1\left[(x-x_1)^{2}+(y-y_1)^{2}\right]}{v_1 \alpha_1^2} - 1)}{u}$$

Similarly for $\alpha_2$
$$\frac{\delta}{\delta \alpha_2} = -\sum \frac {\frac{2\lambda_2(\beta_2 - 1)}{\pi \alpha_2^3 v_2^\beta_2}(\frac{\beta_2\left[(x-x_2)^{2}+(y-y_2)^{2}\right]}{v_2\alpha_2^2} - 1)}{u}$$
...
$$\frac{\delta}{\delta \beta_1} = -\sum \frac{ s_1v_{1}^{-\beta_1}(1-\beta_1\ln(v_1) + \ln(v_1))}{u}$$
Similarly for $\beta_2$
$$\frac{\delta}{\delta \beta_2} = -\sum \frac{ s_2v_{2}^{-\beta_2}(1-\beta_2\ln(v_2) + \ln(v_2))}{u}$$
