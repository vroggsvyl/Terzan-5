...
```
```{r}
interestAreaCS1 <- interestArea %>%
  filter(x>4110 & x<4120 & y>4047 & y<4057)

interestAreaCS2 <- interestArea %>%
  filter(x>4120 & x<4126 & y>4060 & y<4066)

ggplot(interestAreaCS1,aes(x,y)) + 
  geom_pointdensity(adjust = 0.05, size = 0.1, shape = "1") + 
  scale_color_viridis(direction = -1, option = "B", trans = "log", breaks = c(4, 16, 64)) + 
  coord_fixed()

ggplot(interestAreaCS2,aes(x,y)) + 
  geom_pointdensity(adjust = 0.05, size = 0.1, shape = "1") + 
  scale_color_viridis(direction = -1, option = "B", trans = "log", breaks = c(4, 16, 64)) + 
  coord_fixed()

a <- nrow(interestAreaCS1)
b <- nrow(interestAreaCS2)
c <- nrow(interestArea)
...
```{r}
#A Mixture Model for 2 Moffat Distributions
MM2MOFFATnorestrictions <- function(params, x, y){
...
#The partial derivatives of the Negative Log Likelihood
derivNLLMM2MOFFATnorestrictions <- function(params, x, y){
...
Convergence error occurs depending on the input of the inital value of lambda
```{r}
