...
Moffat <- function(parameters, x){
  ...
  amplitude <- pmin(1-1e-6, amplitude)
  gamma <- pmax(gamma, 1e-6)
  alpha <- pmax(alpha, 1e-6)
  #limiting values, amplitude must be between 0 and 1. As I am creating a probability density function, the density must be between 0 and 1 and the amplitude represents the peak of the density.
  #gamma can't be 0 otherwise we run into a math error.
  #alpha can't equal 0 otherwise the pdf will be equal to 1 for everything. It can't be less than zero as that'll cause the density to be greater than 1. 
...
This indicates to me that in order to minimize the negative log likelihood function. Mu should exist near 4115. The amplitude should
...
```{r}
#set initial guesses
initialGuess <- c(amplitude = 0.4, mu = 2, gamma = 1, alpha = 0.5)
#set bounds
lowerBounds <- c(1e-6, -Inf, 1e-6, 1e-6)
upperBounds <- c(1-1e-6, Inf, Inf, Inf)

#sampledata
sampleData <- rnorm(1000, mean = 2, sd = 1)

result <- optim(par = initialGuess, fn = negLogLike, gr = MoffatDeriv, x = sampleData, method = "L-BFGS-B", lower = lowerBounds, upper = upperBounds)
optimPar <- result$par
...
#set initial guesses
initialGuess <- c(amplitude = 0.1, mu = 4115, gamma = 20, alpha = 0.5)

#Method 2
#l2m2 = line 2 (slice of celestial source), method 2 (mle)
resultl2m2 <- optim(par = initialGuess, fn = negLogLike, gr = MoffatDeriv, x = xl2, method = "L-BFGS-B", lower = lowerBounds, upper = upperBounds)
