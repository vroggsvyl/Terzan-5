...
#For the marginal PDF of X, integrate (or in this instance sum) all the values 
...
margY <- ggplot(marginalYdf) +
  geom_line(aes(x = y, y = density), col = cbPalette[2]) +
...
histY <- histY + clean_theme() + rremove("legend") 
histX <- histX + clean_theme() + rremove("legend")
# Arranging the plot using cowplot

plot_grid(margX, NULL, NULL, histX, NULL, NULL, combined, histY, margY, ncol = 3, align = "hv", 
      rel_widths = c(3, 1, 1), rel_heights = c(1, 1, 3))

```
```{r}
marginalWithHist <- ggMarginal(combined, type = "histogram", bins = 100)

plot_grid(margX, NULL, marginalWithHist, margY, ncol = 2, align = "hv", axis = c("l", "b"), 
...
histogramWithMarginalY <- ggplot(POI2, aes(y = y)) +
  geom_histogram(aes(x = after_stat(density)), bins = 100, col = cbPalette[3], fill = cbPalette[5]) +
  geom_line(data = marginalYdf, aes(x = y, y = density), col = cbPalette[2]) +
  labs(x = "Density", y = "y", main = "Marginal PDF of Y") 
...
```
```{r}
source <- ggplot(POI2, aes(x,y)) +
  geom_pointdensity(adjust = 0.05, size = 0.1, shape = "1") + 
  scale_color_viridis(direction = -1, option = "B", trans = "log", breaks = c(10, 100, 1000)) + 
  coord_fixed()

marginalSource <- ggMarginal(source, "histogram")

marginalSource

sourceWithContours <- source + 
  geom_path(data = contourData, aes(x = x, y = y, group = group), color = cbPalette[5], alpha = 1, linewidth= 0.3) 

sourceWithContours

marginalSourceWithContours <- ggMarginal(sourceWithContours, "histogram")

marginalSourceWithContours
```
...
Add marginal theoretical curves 
...
Look at other thesis statements
