...
## Set up
The aim in this section is to find a 1D distribution that is an appropriate fit to the data.

First I sliced the data into smaller cross sections with a height of 1.
```{r}
line0 <- POI2 %>%
...
RAWPOI2 +
  ...
  geom_hline(yintercept = 4054.5)
```
Line2 focuses on all the data points between the two red lines.
From the data I take only the x value of all points and construct a histogram.
...
## 2.2 Fitting to a Normal Distribution
```{r}
...
histWithNormL2
```
Visually Normal doesn't appear to be a good fit.
...
Visually and numerically the Normal Distribution doesn't work.

## 2.3 - Moffat Distribution
...
Convert Moffat Model into Moffat Distribution
...
Optimising the parameters for line 2
```{r}
...
Visually the Moffat Distribution appears to be an appropriate fit
Test numerically.
KS Test and Anderson Darling Test
...
## 2.4 - Comparison
Compar between the performance of the Moffat Distribution and the Normal
...
# Other sections of the Celestial Source

Line 1, 3, 4, 5
Lines 1 and 5 are on the outskirts of the celestial source
Lines 0 and 6 are the very minimum

Lineback is purely the cosmic background.
